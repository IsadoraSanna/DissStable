...
static void setCameraParameters(std::vector<glm::vec3> vertices, glm::vec3 &centre, float &radius);
...
OBJModel::OBJModel(const std::string& fileName)
{
    ...
    setCameraParameters(vertices, centre, radius);

}

void setCameraParameters(std::vector<glm::vec3> vertices, glm::vec3 &centre, float &radius) {
    //initialise the xyz values of the max and min coordinates = infinity
    glm::vec3 minCoord(std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity(), std::numeric_limits<float>::infinity());
    glm::vec3 maxCoord(-std::numeric_limits<float>::infinity(), -std::numeric_limits<float>::infinity(), -std::numeric_limits<float>::infinity());
    radius = 0;

    for (unsigned int i = 0; i < vertices.size(); i++)
    {
        if (vertices[i].x > maxCoord.x)
            maxCoord.x = vertices[i].x;
        else if (vertices[i].x < minCoord.x)
            minCoord.x = vertices[i].x;

        if (vertices[i].y > maxCoord.y)
            maxCoord.y = vertices[i].y;
        else if (vertices[i].y < minCoord.y)
            minCoord.y = vertices[i].y;

        if (vertices[i].z > maxCoord.z)
            maxCoord.z = vertices[i].z;
        else if (vertices[i].z < minCoord.z)
    ...
    {
        if (coordinatesArray[i] > radius)
            radius = coordinatesArray[i];
    }
}
